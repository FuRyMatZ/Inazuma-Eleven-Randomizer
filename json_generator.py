...
IE_betaVR_FolderPath = os.path.abspath(os.path.join(os.getcwd(), "Resource", "Joueurs", "Inazuma Eleven Victory Road Beta"))
...
IE_betaVR_List = [os.path.join(IE_betaVR_FolderPath, fichier) for fichier in os.listdir(IE_betaVR_FolderPath) if os.path.isfile(os.path.join(IE_betaVR_FolderPath, fichier)) and fichier.lower().endswith(".png")]
...
#IE_beta_VicotryRoad
#-----------------------------------------------------------------------
for i,element in enumerate(IE_betaVR_List):
    nom_fichier = os.path.basename(element)
    ...
    IE_betaVR[i] = {
        ...
        'Genre':genre,
        'Poste':poste,
        'Path': os.path.join(IE_betaVR_FolderPath, nom_fichier)
    }
